...
export const ProfileDOB = ({ goBack, user, setUser }) => {
  const [editField, setEditField] = useState(null); // Stores which field is being edited
  const [newValue, setNewValue] = useState(""); // Stores new value for editing
  const modalRef = useRef(null);
  const dispatch = useDispatch();
  const {addProfileDateOfBirth,isLoading} = useProfile();
 ...
 useEffect(() => {
  ...
  const saveChanges = async() => {
    ...
    if(user){
      const response = await addProfileDateOfBirth( newValue);
      console.log("response: ", response)
      setUser({ ...user, [editField]: newValue })
      const dateofbirth = new Date(response?.data?.user.dateofbirth);
      dispatch(updateProfile({ field: "dateofbirth", value: dateofbirth}));
      console.log("DOB",dateofbirth)
      console.log("updated user",typeof(dateofbirth))
